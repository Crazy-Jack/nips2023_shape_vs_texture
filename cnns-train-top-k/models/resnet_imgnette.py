...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, topk_layer=[], topk=0.1, num_classes=10):
        ...
        # print(f"Linear layer input : {512*block.expansion}")
    ...
    def forward(self, x):
        ...
        if 1 in self.topk_layer:
          out = self.sparse_hw(out, tau=1, topk=self.topk)
        out = self.layer2(out)
        
        if 2 in self.topk_layer:
          out = self.sparse_hw(out, tau=1, topk=self.topk)
        out = self.layer3(out)
        
        if 3 in self.topk_layer:
          out = self.sparse_hw(out, tau=1, topk=self.topk)
        out = self.layer4(out)
        
        if 4 in self.topk_layer:
          out = self.sparse_hw(out, tau=1, topk=self.topk)
...
def ResNet18_topk(topk_level, topk_sparsity):
    return ResNet(BasicBlock, [2, 2, 2, 2], topk_level, topk_sparsity)
